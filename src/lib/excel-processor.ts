...
export const MACRO_CONFIGS: Record<string, {
...
export async function processExcelFileLocal(file: File, macroId: string, setProgress: (progress: number) => void): Promise<ProcessedData> {
  const config = MACRO_CONFIGS[macroId];
  if (!config) {
    throw new Error("Macro no valido");
  }
